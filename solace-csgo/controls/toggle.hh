...
public:
	...
	auto draw( ) -> void override {
		...
		if ( *this->enabled_ )
			g.m_render->filled_rect( button_area.x + 1, button_area.y+1, button_area.h - 2, button_area.h-2, { 0xDB, 0x2E, 0x2C, 100 });
		g.m_render->outlined_rect( button_area.x, button_area.y, button_area.h, button_area.h, { 240,240,240, 7 } );
		auto text_height = g.m_render->get_text_height( this->name, g.m_render->m_constantia_12( ) );
		g.m_render->text( g.m_render->m_constantia_12( ), this->area.x, this->area.y + (area.h / 2) - text_height/2, { 240,240,240, 100 }, this->name );
	}
