...
void esp_t::run()
{
    if (!g.m_local)
        return;
    auto weapon =
        static_cast<weapon_t*>(g.m_interfaces->entity_list()->get_client_entity_handle(g.m_local->active_weapon()));
    if (weapon)
...
void esp_t::smoke(entity_t* ent)
{
    ...
    else
    {
        ...
        if (!math::world_to_screen(ent->abs_origin(), screen))
            return;
        g.m_render->text(g.m_render->m_tahoma_14(), screen.x, screen.y, menu.dark_accent, "SMOKE",
                         Horizontal | Vertical);
    }
}
...
void esp_t::inferno(entity_t* ent)
{
    ...
    auto* thrower =
        static_cast<player_t*>(g.m_interfaces->entity_list()->get_client_entity_handle(inferno->m_thrower()));
    bool team = false;
    if (thrower)
    ...
    for (std::size_t i = 0; i < delaunator.triangles.size(); i += 3)
    {
        ...
        point = vec3_t(static_cast<float>(delaunator.coords[3 * delaunator.triangles[i]]),
                       static_cast<float>(delaunator.coords[3 * delaunator.triangles[i] + 1]),
                       static_cast<float>(delaunator.coords[3 * delaunator.triangles[i] + 2]));
        vec3_t screen;
        if (!math::world_to_screen(point, screen))
            continue;
        verts[0] = render_t::vertex_t{screen.x, screen.y, 0, 1, point_color};

        point = vec3_t(static_cast<float>(delaunator.coords[3 * delaunator.triangles[i + 1]]),
                       static_cast<float>(delaunator.coords[3 * delaunator.triangles[i + 1] + 1]),
                       static_cast<float>(delaunator.coords[3 * delaunator.triangles[i + 1] + 2]));
        if (!math::world_to_screen(point, screen))
            continue;
        verts[1] = render_t::vertex_t{screen.x, screen.y, 0, 1, point_color};

        point = vec3_t(static_cast<float>(delaunator.coords[3 * delaunator.triangles[i + 2]]),
                       static_cast<float>(delaunator.coords[3 * delaunator.triangles[i + 2] + 1]),
                       static_cast<float>(delaunator.coords[3 * delaunator.triangles[i + 2] + 2]));
        if (!math::world_to_screen(point, screen))
            continue;
...
void esp_t::NoSmoke()
{
    ...
    if (!smoke1)
        smoke1 = g.m_interfaces->material_system()->find_material("particle/vistasmokev1/vistasmokev1_fire",
                                                                  "Other textures");

    if (!smoke2)
        smoke2 = g.m_interfaces->material_system()->find_material("particle/vistasmokev1/vistasmokev1_smokegrenade",
                                                                  "Other textures");

    if (!smoke3)
        smoke3 = g.m_interfaces->material_system()->find_material("particle/vistasmokev1/vistasmokev1_emods",
                                                                  "Other textures");

    if (!smoke4)
        smoke4 = g.m_interfaces->material_system()->find_material("particle/vistasmokev1/vistasmokev1_emods_impactdust",
                                                                  "Other textures");
...
bool esp_t::get_player_box(player_t* player, area_t* box)
{
    ...
    const auto& info = g_player_manager.m_ents[player->index() - 1];
    if (info.m_records.size() >= 2)
    {
        if (auto const& record = info.m_records.front())
        {
    ...
    const auto size = g.m_render->m_screen_size();
...
void esp_t::offscreen(player_t* ent)
{
    ...
    render_t::vertex_t triag[3] = {
        {screen_size.Width / 2.f + cos(yaw_delta) * (r_x), screen_size.Height / 2.f + sin(yaw_delta) * (r_y), 0, 1,
         clr},
        {screen_size.Width / 2.f + (cos(yaw_delta) * (r_x - triangle_size)) +
             (cos(yaw_delta + DEG2RAD(90)) * triangle_size),
         screen_size.Height / 2.f +
             (sin(yaw_delta) * (r_y - triangle_size) + (sin(yaw_delta + DEG2RAD(90)) * triangle_size)),
         0, 1, clr},
        {screen_size.Width / 2.f + (cos(yaw_delta) * (r_x - triangle_size)) +
             (cos(yaw_delta + DEG2RAD(90)) * -triangle_size),
         screen_size.Height / 2.f +
...
void esp_t::player(player_t* ent)
{
    ...
    if (settings::visuals::players::box)
    {
        ...
        g.m_render->outlined_rect(box.x, box.y, box.w, box.h,
                                  on_team ? color{0xFF, 0x08, 0xFF} : color{0x81, 0xFF, 0x21});
        g.m_render->outlined_rect(box.x + 1, box.y + 1, box.w - 2, box.h - 2, color(0, 0, 0, 200));
    }
    if (settings::visuals::players::health)
    {
        ...
        g.m_render->outlined_rect(health_box.x - 1, health_box.y - 1, health_box.w + 2, health_box.h + 2,
                                  color(50, 50, 50, 150));
        ...
        g.m_render->filled_rect(health_box.x, health_box.y + health_box.h * (1 - health_percent), health_box.w,
                                health_box.h * health_percent,
                                color(255.f * (1 - health_percent), 255.f * health_percent, 0));
    }
    if (settings::visuals::players::name)
    {
        ...
        g.m_render->text(g.m_render->m_tahoma_12(), box.x + box.w / 2.f, box.y - (height + 1), color(255, 255, 255),
                         info.name, Horizontal);
    }
    if (settings::visuals::players::weapon)
    {
        auto* weapon =
            static_cast<weapon_t*>(g.m_interfaces->entity_list()->get_client_entity_handle(ent->active_weapon()));
        if (weapon)
        {
            ...
            if (weapon_data)
            {
                ...
                if (name.find("weapon_") != std::string::npos)
                    name = name.substr(7);
                g.m_render->text(g.m_render->m_tahoma_12(), box.x + box.w / 2.f, box.y + box.h + 1,
                                 color(255, 255, 255), name.c_str(), Horizontal);
            }
        }
    }
