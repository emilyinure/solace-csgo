...
void _stdcall paint_traverse(long panel, bool repaint, bool force) {
  ...
  if (!zoom && fnv::hash(g.m_interfaces->panel()->GetName(panel)) ==
                   fnv::hash("HudZoom"))
    zoom = panel;

  if (panel == zoom && settings::visuals::weapons::noscope) return;

  using PaintTraverse_t = void(__thiscall *)(void *, long, bool, bool);
  g.m_interfaces->panel().hook()->get_original<PaintTraverse_t>(
      panel_t::PAINTTRAVERSE)(g.m_interfaces->panel().operator void *(), panel,
                              repaint, force);
}

bool _stdcall create_move(float input_sample_frametime, cmd_t *cmd) {
  g.m_interfaces->client_mode()
      .hook()
      ->get_original<bool(_stdcall *)(float, cmd_t *)>(24)(
...
long __stdcall end_scene(IDirect3DDevice9 *pDevice) {
  static auto end_scene_target =
      g.m_interfaces->device().hook()->get_original<decltype(&end_scene)>(42);
  if (!pDevice) return end_scene_target(pDevice);
...
LRESULT __stdcall wndproc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  if (uMsg == WM_MOUSEMOVE)
    input_helper.set_mouse_position({static_cast<float>(GET_X_LPARAM(lParam)),
                                     static_cast<float>(GET_Y_LPARAM(lParam))});
  if (uMsg == WM_MOUSEWHEEL)
    input_helper.set_scroll_position(
...
void __fastcall update_anims(player_t *player, uint32_t edx) {
  ...
  if (player == g.m_local) {
    if (g.m_running_client)
      ...
      vmt->get_original<void(__fastcall *)(player_t *, uint32_t)>(218)(player,
                                                                       edx);
      //__asm {
      //	mov     ecx, player
      //	call original
...
bool __fastcall setupbones(animating_t *anim, uint32_t edx, matrix_t *out,
                           int max, int mask, float time) {
  auto player =
      reinterpret_cast<player_t *>(reinterpret_cast<uintptr_t>(anim) - 0x4);
  auto *vmt = g.m_hooks->players_hook(player->index() - 1);
  ...
  return vmt
      ->get_original<bool(__thiscall *)(void *, matrix_t *, int, int, float)>(
          13)(player, out, max, mask, time);
}

bool __fastcall is_player(player_t *player, uint32_t edx) {
  auto *vmt = g.m_hooks->players_hook(player->index() - 1);
  static auto shouldskipanim =
      util::find("client.dll", "84 C0 75 ? 5F C3 8B 0D ? ? ? ?");
...
void __fastcall AllocateIntermediateData(player_t *_this, uint32_t edx) {
  g.m_local =
      static_cast<player_t *>(g.m_interfaces->entity_list()->get_client_entity(
...
void __fastcall draw_model_execute(void *this_ptr, uint32_t edx, uintptr_t ctx,
                                   void *state, const model_render_info_t &info,
                                   matrix_t *bone) {
  static auto original_draw_model =
      reinterpret_cast<decltype(&draw_model_execute)>(
          g.m_interfaces->model_render().hook()->get_original(21));

  if (!info.pRenderable)
    return original_draw_model(this_ptr, edx, ctx, state, info, bone);

  auto *const base_entity = static_cast<entity_t *>(
      g.m_interfaces->entity_list()->get_client_entity(info.entity_index));

  if (!base_entity || !base_entity->is_player())
    return original_draw_model(this_ptr, edx, ctx, state, info, bone);

  g_chams.player(reinterpret_cast<player_t *>(base_entity), ctx, state, info,
                 bone);
}
...
void __stdcall override_view(view_setup_t *view) {
  ...
  if (alive) {
    //g_grenade_pred.View();
    if (settings::misc::misc::thirdperson &&
        !g.m_interfaces->input()->camera_is_third_person())
      g.m_interfaces->input()->camera_to_third_person();
    else if (!settings::misc::misc::thirdperson &&
             g.m_interfaces->input()->camera_is_third_person()) {
  ...
  if (g.m_interfaces->input()->camera_is_third_person()) {
...

    {
      ...
      g.m_interfaces->trace()->trace_ray(
          ray_t(origin, origin - (forward * offset.z), {-16.f, -16.f, -16.f},
                {16.f, 16.f, 16.f}),
  ...
  g.m_interfaces->client_mode().hook()->get_original<decltype(&override_view)>(
      18)(view);
}
...
int __fastcall SendDatagram(i_net_channel *this_ptr, uint32_t edx, void *data) {
  ...
  if (settings::misc::misc::fake_latency) {
    ...
    for (const auto &sequence : g.m_sequences) {
      if (g.m_interfaces->globals()->m_curtime - sequence.m_time >=
          ping / 1000.f) {
...
void __fastcall SetChoked(i_net_channel *this_ptr, uint32_t edx) {
  ...
  auto local =
      static_cast<player_t *>(g.m_interfaces->entity_list()->get_client_entity(
          g.m_interfaces->engine()->local_player_index()));
  if (ret != addr || !local || !local->alive() || !g.m_valid_round ||
      !g.m_interfaces->engine()->is_connected() ||
...
bool __fastcall TempEntities(client_state_t *this_ptr, uint32_t edx,
                             void *msg) {
  if (!g.m_running_client) {
...
c_hook view_render;
void __fastcall RenderSmokeOverlay(i_render_view *this_ptr, uint32_t edx,
                                   bool unk) {
...
void m_nSmokeEffectTickBegin(c_recv_proxy_data *pData, void *pStruct,
                             void *pOut) {
  if (!pData || !pStruct || !pOut) return;
...
void Body_proxy(c_recv_proxy_data *data, address ptr, address out) {
  address stack{reinterpret_cast<uintptr_t>(_AddressOfReturnAddress()) -
                sizeof(uintptr_t)};
  ...
  if ((stack.get(2) + sizeof(uintptr_t)) != RecvTable_Decode) {
    ...
    if (player) {
      ...
      if (ent) g_resolver.OnBodyUpdate(ent, data->m_value.m_Float);
    }
  }

  // call original proxy.
  if (m_Body_original) m_Body_original(data, ptr, out);
}
...
bool __fastcall NetShowFragmentsGetBool(void *this_ptr, uint32_t edx) {
  ...
  static auto read_sub_channel_data_ret =
      static_cast<address>(
          util::find(
              "engine.dll",
              "85 C0 74 12 53 FF 75 0C 68 ? ? ? ? FF 15 ? ? ? ? 83 C4 0C"))
          .as<uintptr_t *>();
  static auto check_receiving_list_ret =
      static_cast<address>(
          util::find("engine.dll", "8B 1D ? ? ? ? 85 C0 74 16 FF B6"))
          .as<uintptr_t *>();

  static uint32_t last_fragment = 0;

  if (_ReturnAddress() == reinterpret_cast<void *>(read_sub_channel_data_ret) &&
      last_fragment > 0) {
    const auto data = &reinterpret_cast<uint32_t *>(
        g.m_interfaces->client_state()->m_net_channel)[0x56];
...
void __stdcall run_command(player_t *player, cmd_t *cmd,
                           player_move_helper *helper) {
  g.m_local =
      static_cast<player_t *>(g.m_interfaces->entity_list()->get_client_entity(
          g.m_interfaces->engine()->local_player_index()));
  typedef void(__thiscall * o_run_command)(void *, player_t *, cmd_t *,
                                           player_move_helper *);
  // if ( player == g.m_local ) {
  //	prediction::finish_partial_frame( player, cmd );
  //}
  // else
  g.m_interfaces->prediction().hook()->get_original<o_run_command>(19)(
      g.m_interfaces->prediction(), player, cmd, helper);
}

bool __stdcall InPrediction() {
  typedef void(__thiscall * o_run_command)(void *);
  if (g.m_in_pred) return true;
  g.m_interfaces->prediction().hook()->get_original<o_run_command>(14)(
      g.m_interfaces->prediction());
}

void __stdcall PreEntityPacketReceived(int commands_acknowledged,
  ...
  g.m_interfaces->prediction().hook()->get_original<o_PostNetworkDataReceived>(
      4)(g.m_interfaces->prediction(), commands_acknowledged,
         current_world_update_packet, server_ticks_elapsed);
}
void __stdcall PostNetworkDataReceived(int commands_acknowledged) {
  ...
  if (g.m_local) {
    ...
    if (map) {
      if (!g.m_map_setup) {
        ...
        map->m_packed_size = 0;
        g.m_map_setup = CPredictionCopy::PrepareDataMap(map);
        if (g.m_map_setup) g_pred_manager.init(map);
      }
    }
  }
  if (g.m_local && g.m_local->alive()) {
  ...
  g.m_interfaces->prediction().hook()->get_original<o_PostNetworkDataReceived>(
      6)(g.m_interfaces->prediction(), commands_acknowledged);
  g.net_data_received();
}
...
bool __fastcall FireEventClientSide(void *ecx, void *edx, IGameEvent *pEvent) {
  if (strcmp(pEvent->GetName(), "bullet_impact")) g_resolver.on_impact(pEvent);
  return g.m_interfaces->events().hook()->get_original<FireEventFn>(8)(
      g.m_interfaces->events(), pEvent);
}

void __stdcall SceneEnd() {
  g.m_interfaces->render_view()
      .hook()
      ->get_original<void(__thiscall *)(void *)>(9)(
          g.m_interfaces->render_view());
  g_chams.SceneEnd();
}

void __fastcall PacketStart(client_state_t *this_ptr, void *edx,
                            int incoming_sequence, int outgoing_acknowledged) {
  auto local =
      static_cast<player_t *>(g.m_interfaces->entity_list()->get_client_entity(
          g.m_interfaces->engine()->local_player_index()));
  using packet_start_t = void(__thiscall *)(client_state_t *, int, int);
  if (!local || !local->alive() || !g.m_valid_round ||
      !g.m_interfaces->engine()->is_connected() ||
      !g.m_interfaces->engine()->in_game())
    return m_cl_hook.get_original<packet_start_t>(5)(
        this_ptr, incoming_sequence, outgoing_acknowledged);
  ;
...
float __fastcall Hook_GetScreenAspectRatio(void *pEcx, void *pEdx,
                                           int32_t iWidth, int32_t iHeight) {
  if (settings::visuals::misc::aspectratio != 0)
    iWidth = int(iWidth * ((settings::visuals::misc::aspectratio) / 100.f));
  return g.m_interfaces->engine()
      .hook()
      ->get_original<float(__thiscall *)(void *, int32_t, int32_t)>(101)(
          pEcx, iWidth, iHeight);
}
void __stdcall Hooked_ClientCmd(const char *str, bool force) {
  ...
  if (name.length() > 5 && strcmp(name.substr(0, 5).c_str(), "save ") == 0) {
    ...
    file.close();
             ...
             strcmp(name.substr(0, 5).c_str(), "load ") == 0) {
    name = name.substr(5, name.length() - 1) + ".cfg";
    ...
    g.m_interfaces->engine()
        .hook()
        ->get_original<void(__thiscall *)(void *, const char *, bool)>(114)(
            g.m_interfaces->engine(), str, force);
}

long __stdcall reset(LPDIRECT3DDEVICE9 device,
                     D3DPRESENT_PARAMETERS *presentation_parameters) {
  g.m_render->on_lost_device();
  auto result = g.m_interfaces->device().hook()->get_original<decltype(&reset)>(
      16)(device, presentation_parameters);
...
void __stdcall LockCursor() {
  ...
  bool state = true;
  if (!g.m_interfaces->engine()->in_game() ||
      (g.m_local && !g.m_local->alive())) {
    state = !menu.open;
  }

  g.m_interfaces->input_system()->EnableInput(state);

  if (menu.open) g.m_interfaces->surface()->UnlockCursor();
}
hooks_t::hooks_t() {
  while (!((g.m_window = FindWindowA("Valve001", nullptr)))) Sleep(100);

  g_chams.create_materials();

  m_custom_entity_listener.init();
  auto net_show_fragments_v =
      g.m_interfaces->console()->get_convar("net_showfragments");
  net_show_fragments.init((uintptr_t)net_show_fragments_v);
  net_show_fragments.add(static_cast<void *>(NetShowFragmentsGetBool), 13);

  g.m_old_window = reinterpret_cast<WNDPROC>(SetWindowLongPtr(
      g.m_window, GWL_WNDPROC, reinterpret_cast<LONG_PTR>(wndproc)));
  ...
  oLockCursor =
      (lock_cursor_t)g.m_interfaces->surface().hook()->add(LockCursor, 67);
  g.m_interfaces->client().hook()->add(LevelInitPostEntity, 6);
  ...
  m_cl_hook.init(
      reinterpret_cast<uintptr_t>(g.m_interfaces->hookable_client_state()));
  ...
  netvar_manager::set_proxy(fnv::hash("DT_CSPlayer"),
                            fnv::hash("m_flLowerBodyYawTarget"), Body_proxy,
                            m_Body_original);
  netvar_manager::set_proxy(
      fnv::hash("DT_SmokeGrenadeProjectile"), fnv::hash("m_bDidSmokeEffect"),
  ...
  original_cl_move = reinterpret_cast<decltype(&cl_move)>(DetourFunction(
      util::find("engine.dll",
                 "55 8B EC 81 EC ? ? ? ? 53 56 57 8B 3D ? ? ? ? 8A F9"),
