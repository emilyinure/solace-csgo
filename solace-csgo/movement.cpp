...
void movement::draw() {
  ...
  if (radius > 0) {
    ...
    for (auto i = 0; i < 360; i += 5) {
      ...
      verts[1] = {roundf(point_wts_2.x), roundf(point_wts_2.y), 0, 1,
                  point_color};
      point_color.set_a(100);
      verts[2] = {roundf(origin_w2s.x), roundf(origin_w2s.y), 0, 1,
                  point_color};
      g.m_render->render_triangle(verts, 1);
      point_color.set_a(50);
      g.m_render->line(roundf(point_wts.x), roundf(point_wts.y),
                       roundf(point_wts_2.x), roundf(point_wts_2.y),
                       point_color, 1);
    }
  }
}
...
void movement::bhop() {
  if (!settings::misc::movement::bhop) return;
  if (!(g.m_cmd->m_buttons & IN_JUMP)) return;
...
void movement::PreciseMove() {
  if (!g.m_interfaces->entity_list()->get_client_entity_handle(
    ...
    return;
  if (g.m_cmd->m_buttons & IN_JUMP) return;
  if (g.m_cmd->m_forwardmove != 0.f || g.m_cmd->m_sidemove != 0.f) return;
  static auto sv_friction =
      g.m_interfaces->console()->get_convar("sv_friction");
  static auto sv_stopspeed =
      g.m_interfaces->console()->get_convar("sv_stopspeed");
...
void movement::auto_peek() {
  ...
  if (settings::hvh::antiaim::auto_peek) {
    ...
    if (m_should_unpeek) m_invert = true;
...
void movement::move_to(vec3_t target_origin) const {
  ...
  if (speed > 0.1f) {
    const auto friction =
        sv_friction->GetFloat() * g.m_local->surface_friction();
  ...
  auto local_delta = (target_origin - local_origin) /
  ...
  static auto sv_accelerate =
      g.m_interfaces->console()->get_convar("sv_accelerate");

  float max_accel = 450;
  auto accel_speed = sv_accelerate->GetFloat() *
                     g.m_interfaces->globals()->m_interval_per_tick * 450;
  if (max_accel > accel_speed) max_accel = accel_speed;
  ...
  if (g.m_weapon_info && g.m_weapon_info->m_max_player_speed > 0.f) {
    ...
    if (fRatio > 0.f) projected_delta /= fRatio;
  }

  g.m_view_angles = ang_t(
      0,
      static_cast<float>(atan2(local_delta.y, local_delta.x) * (180.f / M_PI)),
      0);
  ;
  ...
  if (max_accel <= add_speed) {
    ...
    if (accel_speed <= add_speed) {
      ...
      projected_delta =
          projected_delta / (sv_accelerate->GetFloat() *
                             g.m_interfaces->globals()->m_interval_per_tick);
    }
...
{
  if (g.m_local->velocity().z >= -7.f &&
      floorf(g.m_local->velocity().z) != 7.f &&
...
void movement::edge_bug() {
  ...
  if (map) {
    ...
    if (!found) {
      ...
      for (auto i = 0;
           i < 1 / (g.m_interfaces->globals()->m_interval_per_tick / 10.f);
           i++) {
      ...
      if (ran) prediction::end();
    }

    CopyHelper =
        CPredictionCopy(PC_EVERYTHING, reinterpret_cast<byte*>(g.m_local),
                        false, static_cast<const byte*>(startdata), true,
                        CPredictionCopy::TRANSFERDATA_COPYONLY);
...
void movement::auto_strafe() {
  if (settings::misc::movement::autostrafe == 0) return;

  if (!(g.m_cmd->m_buttons & IN_JUMP) && !m_force_strafe) return;
  ...
  g.m_view_angles.y +=
      static_cast<float>(atan2(direction.y, direction.x) * (180.f / M_PI));
  ...
  if (abs_delta <= ideal2 || abs_delta >= 30.f) {
    ...
    while (velocity_delta > 180) velocity_delta -= 360;
    while (velocity_delta < -180) velocity_delta += 360;

    // correct our strafe amongst the path of a circle.
    if (fabsf(velocity_delta) > ideal2) {
      g.m_view_angles.y =
          velocity_angle - std::copysignf(ideal2, velocity_delta);
...
class pre_speed_node {
  ...
  pre_speed_node(float angle, vec3_t pos, vec3_t vel)
      : angle(angle), position(pos), velocity(vel) {}
  int recur(int& iter) {}
};
...
void movement::DoPrespeed() {
  ...
  while (true) {
    ...
    if (!WillCollide(time, step, m_circle_yaw) || max <= step) break;
  ...
  if (step > max) {
    ...
    while (true) {
      // if we will not collide with an object or we wont accelerate from such a
      // big step anymore then stop.
      if (!WillCollide(time, step, m_circle_yaw) || step <= 0.f) break;
...
inline bool movement::GetClosestPlane(vec3_t& plane) {
  ...
  for (float step{}; step <= M_PI_2_F; step += (M_PI_F / 10.f)) {
    ...
    g.m_interfaces->trace()->trace_ray(ray_t(start, end, m_mins, m_maxs),
...
void air_acceletate(vec3_t& wishdir, vec3_t& velocity, const float wishspeed,
                    const float accel) {
  ...
  float accelspeed =
      accel * wishspeed * g.m_interfaces->globals()->m_interval_per_tick;
 ...
 public:
  ...
  virtual bool ShouldHitEntity(void* pEntityHandle, int contentsMask) {
    return (((entity_t*)pEntityHandle)->index() == 0 ||
            ((entity_t*)pEntityHandle)->index() >=
                g.m_interfaces->globals()->m_max_clients);
  }
...
void try_touch_ground(const vec3_t& start, const vec3_t& end,
...
void movement::try_touch_ground_in_quadrants(const vec3_t& start,
                                             const vec3_t& end,
                                             unsigned int fMask, trace_t& pm) {
  vec3_t maxs;
...
inline bool movement::WillCollide(float time, float change, float start) {
  ...
  static auto sv_staminajumpcost =
      g.m_interfaces->console()->get_convar("sv_staminajumpcost");
  static auto sv_staminalandcost =
      g.m_interfaces->console()->get_convar("sv_staminalandcost");
  static auto sv_staminarecoveryrate =
      g.m_interfaces->console()->get_convar("sv_staminarecoveryrate");
  static auto sv_staminamax =
      g.m_interfaces->console()->get_convar("sv_staminamax");
  ...
  for (data.predicted = 0.f; data.predicted < time;
       data.predicted += g.m_interfaces->globals()->m_interval_per_tick) {
    ...
    ang_t(0, data.direction, 0)
        .vectors(&forward, &right, &up);  // Determine movement angles
    ...
    static auto sv_accelerate =
        g.m_interfaces->console()->get_convar("sv_airaccelerate");
    air_acceletate(wishdir, data.velocity, wishspeed,
                   sv_accelerate->GetFloat());

    // assume we bhop, set upwards impulse.
    static auto sv_jump_impulse =
        g.m_interfaces->console()->get_convar("sv_jump_impulse");
    static auto sv_gravity =
        g.m_interfaces->console()->get_convar("sv_gravity");
    if (data.ground) {
      stamina = std::clamp<float>(
          stamina + sv_staminalandcost->GetFloat() * data.velocity.z, 0.0f,
          sv_staminamax->GetFloat());
      stamina = std::clamp<float>(stamina + sv_staminajumpcost->GetFloat() *
                                                sv_jump_impulse->GetFloat(),
                                  0.0f, sv_staminamax->GetFloat());
      data.velocity.z = sv_jump_impulse->GetFloat();
    }

    else
      data.velocity.z -= sv_gravity->GetFloat() *
                         g.m_interfaces->globals()->m_interval_per_tick * 0.5f;
    ...
    data.end +=
        (data.velocity * g.m_interfaces->globals()->m_interval_per_tick);

    // trace
    g.m_interfaces->trace()->trace_ray(
        ray_t(data.start, data.end, m_mins, m_maxs), MASK_PLAYERSOLID, &filter,
        &trace);

    // check if we hit any objects.
    if (trace.flFraction != 1.f && trace.plane.normal.z <= 0.9f &&
        trace.plane.normal.z > -.9f)
      return true;
    ...
    try_touch_ground(data.start, data.end - vec3_t{0.f, 0.f, 2.f}, m_mins,
                     m_maxs, MASK_PLAYERSOLID, trace);
    if (trace.plane.normal[2] < 0.7f) {
      // Test four sub-boxes, to see if any of them would have found shallower
      // slope we could actually stand on
      try_touch_ground_in_quadrants(data.start,
                                    data.end - vec3_t{0.f, 0.f, 2.f},
