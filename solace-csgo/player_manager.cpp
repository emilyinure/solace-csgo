...
bool player_record_t::valid() const {
  if (!g.m_interfaces->client_state()->m_net_channel) return false;
  // use prediction curtime for this.
  float curtime = g.ticks_to_time(g.m_local->tick_base());
  static auto* sv_maxunlag =
      g.m_interfaces->console()->get_convar("sv_maxunlag");
  if (floorf(curtime - sv_maxunlag->GetFloat()) > m_pred_time) return false;
  ...
  if (nci) correct += nci->GetLatency(1);
...
player_record_t::player_record_t(ent_info_t* info, float last_sim) {
  ...
  m_bones = static_cast<bone_array_t*>(
      g.m_interfaces->mem_alloc()->alloc(sizeof(bone_array_t) * 128));
}
...
const char* const g_szWeaponPrefixLookupTable[] = {
    "knife",  "pistol", "smg", "rifle",   "shotgun",
    "sniper", "heavy",  "c4",  "grenade", "knife"};
const char* GetWeaponPrefix(std::shared_ptr<player_record_t> record) {
  static auto sig =
      util::find("client.dll", "53 56 8B F1 57 33 FF 8B 4E ? 8B 01");
  return (
      (const char*(__thiscall*)(void*))sig)(record->m_ent->get_anim_state());
  int nWeaponType = 0;  // knife

  auto m_pWeapon =
      (weapon_t*)g.m_interfaces->entity_list()->get_client_entity_handle(
          record->m_ent->active_weapon());
  if (m_pWeapon) {
    auto m_weapon_info = g.m_interfaces->weapon_system()->get_weapon_data(
        m_pWeapon->item_definition_index());
    if (m_weapon_info) {
...
float ent_info_t::test_velocity(std::shared_ptr<player_record_t> record,
                                float speed, float pVec) {
  auto weapon =
      (weapon_t*)g.m_interfaces->entity_list()->get_client_entity_handle(
          record->m_ent->active_weapon());
  float max_speed = record->m_ent->max_speed();
  auto info = weapon ? g.m_interfaces->weapon_system()->get_weapon_data(
...
float ent_info_t::solve_velocity_len(std::shared_ptr<player_record_t> record) {
  ...
  if (comp_value == 0.f) return -1;
  float best = -1;
  float closest = -1;

  for (float i = 0.f; i < max_speed; i += 0.5f) {
    float val =
        SimpleSplineRemapValClamped(i / max_speed, 0.55f, 0.9f, 1.0f, 0.f);
  ...
  if (!state) return record->m_anim_velocity.length();
  float cycle_dif = record->m_layers[6].m_playback_rate;
  if (cycle_dif == 0.f) {
    if (record->m_layers[6].m_cycle >= 0.999f) {
      return (record->m_anim_velocity.length_2d_sqr() != 0.f)
                 ? record->m_anim_velocity.length_2d()
                 : 0.f;
    }
    return 0.f;
  }
  ...
  float m_flWalkToRunTransition =
      *(float*)(record->m_ent->get_anim_state() + 284);
  ...
  if (!set) m_flWalkToRunTransition = fmaxf(0.99f, m_flWalkToRunTransition);

  float m_flAnimDuckAmount = std::clamp(
      Approach(std::clamp(record->m_duck +
                              *(float*)(record->m_ent->get_anim_state() + 168),
                          0.f, 1.f),
               *(float*)(record->m_ent->get_anim_state() + 164),
               g.ticks_to_time(record->m_lag) * 6.0f),
      0.f, 1.f);

  record->m_poses[(int)PoseParam_t::MOVE_BLEND_WALK] =
      (1.0f - m_flWalkToRunTransition) * (1.0f - m_flAnimDuckAmount);
  record->m_poses[(int)PoseParam_t::MOVE_BLEND_RUN] =
      (m_flWalkToRunTransition) * (1.0f - m_flAnimDuckAmount);
  record->m_poses[(int)PoseParam_t::AIM_BLEND_CROUCH_WALK] = m_flAnimDuckAmount;

  vec3_t pVec1;
  pVec1 = record->m_ent->GetSequenceLinearMotion(hdr, nWeaponMoveSeq,
                                                 record->m_poses);
  for (auto i = 0; i < 3; i++)
  ...
  float m_flWalkToRunTransition_1 =
      *(float*)(record->m_ent->get_anim_state() + 284);
  ...
  if (set) m_flWalkToRunTransition_1 = fmaxf(0.01f, m_flWalkToRunTransition_1);

  record->m_poses[(int)PoseParam_t::MOVE_BLEND_WALK] =
      (1.0f - m_flWalkToRunTransition_1) * (1.0f - m_flAnimDuckAmount);
  record->m_poses[(int)PoseParam_t::MOVE_BLEND_RUN] =
      (m_flWalkToRunTransition_1) * (1.0f - m_flAnimDuckAmount);

  vec3_t pVec2;
  pVec2 = record->m_ent->GetSequenceLinearMotion(hdr, nWeaponMoveSeq,
                                                 record->m_poses);
  for (auto i = 0; i < 3; i++)
    if (isnan(fabsf(pVec1[i])))
      return (record->m_anim_velocity.length_2d_sqr() != 0.f)
  ...
  while (test_1 <= max_x_y) {
    float w =
        flMoveCycleRate /
        (float)(1.0 - (float)((test_1 > 135.2) ? m_flWalkToRunTransition
                                               : m_flWalkToRunTransition_1) *
                          0.14999998);
    float val =
        test_velocity(record, test_1, (test_1 > 135.2) ? pVec1_len : pVec_len);
    if (val != -1) {
...
void ent_info_t::UpdateAnimations(std::shared_ptr<player_record_t> record) {
  ...
  if (record->m_lag > 0 && record->m_lag < 16 && m_records.size() >= 2) {
    ...
    if (previous && !previous->m_dormant) {
      record->m_velocity = (record->m_origin - previous->m_origin) *
                           (1.f / g.ticks_to_time(record->m_lag));
  ...
  if (record->m_lag > 1 && !bot) {
    ...
    if (speed > 0.1f && record->m_layers[6].m_weight == 0.0f &&
        record->m_layers[6].m_playback_rate < 0.0001f &&
        record->m_layers[12].m_weight > 0.0f && speed < 10.f &&
    ...
    if (record->m_fake_walk) record->m_anim_velocity = vec3_t(0, 0, 0);
    // we need atleast 2 updates/records
    // to fix these issues.
    if (m_records.size() >= 2 && m_records[1]) {
      if (speed < 20.f && record->m_layers[6].m_weight != 1.0f &&
      ...
      //		record->m_ukn_vel = true;
      //}
      if (record->m_ukn_vel) record->m_anim_velocity = vec3_t(0, 0, 0);
      // get pointer to previous record.
      auto const& previous = m_records[1];
      ...
      if (previous && !previous->m_dormant) {
        ...
        if (!record->m_fake_walk && !record->m_ukn_vel) {
          ...
          // accel per tick.
          vec3_t accel =
              (velo / time) * g.m_interfaces->globals()->m_interval_per_tick;
  ...
  m_ent->client_side_anim() = true;
  {
    ...
    if (!m_teamate) {
      ...
      bool lby_or_move = (!record->m_body_reliable &&
                          record->m_mode == resolver::RESOLVE_BODY) ||
                         record->m_mode == resolver::RESOLVE_WALK;
      if (lby_or_move) {
        mode_data = &m_resolver_data.m_mode_data[resolver_data::LBY_MOVING];
        float val = 30.f;
        if (record->m_mode == resolver::RESOLVE_WALK) val += 20.f;
      ...
      if (mode_data && record->m_resolver_data.m_dir_data.size() ==
                           mode_data->m_dir_data.size()) {
        for (auto i = 0; i < mode_data->m_dir_data.size(); i++) {
          ...
          m_ent->eye_angles().y =
              math::normalize_angle(record_dir_data.angles, 180);
...
void ent_info_t::update(player_t* ent) {
  ...
  if (m_dormant) {
    ...
    if (!m_records.empty()) {
      ...
      if (front && front->m_dormant) insert = false;
    }
  ...
  if (update) {
    ...
    if (!m_teamate) {
      auto* weapon = static_cast<weapon_t*>(
          g.m_interfaces->entity_list()->get_client_entity_handle(
              m_ent->active_weapon()));
          ...
          if (weapon_studio_hdr) {
            for (auto i = 0; i < 15; i++) {
              ...
              if (pLayer->m_sequence <= 1 || pLayer->m_cycle <= 0.f) {
                m_ent->UpdateDispatchLayer(pLayer, weapon_studio_hdr,
                                           pLayer->m_sequence);
              }
            }
          }
      ...
      if (((!current->m_body_reliable &&
            current->m_mode == resolver::RESOLVE_BODY) ||
           current->m_mode == resolver::RESOLVE_WALK) ||
          (current->m_mode == resolver::RESOLVE_STAND1 ||
           current->m_mode == resolver::RESOLVE_STAND2)) {
  ...
  while (m_records.size() > 256) m_records.pop_back();
}
...
void player_manager_t::update() {
  ...
  for (auto i{1}; i <= g.m_interfaces->globals()->m_max_clients; ++i) {
    auto* const player = static_cast<player_t*>(
        g.m_interfaces->entity_list()->get_client_entity(i));
    auto* data = &m_ents[i - 1];
    data->m_valid =
        player && player != g.m_local && player->is_player() && player->alive();
    if (!data->m_valid) {
    ...
    }
    {
      ...
      g_thread_handler.QueueJob([ptr] { ptr->run(); });
    }
  ...
  while (!all_finished) {
    all_finished = true;
    for (auto& i : objects)
      if (!i->finished) all_finished = false;
  }
  // g_thread_handler.start( );
...
bool ent_info_t::build_fake_bones(std::shared_ptr<player_record_t> current) {
  ...
  if (current->m_mode == resolver::RESOLVE_STAND1)
    ...
    mode_data = &m_resolver_data.m_mode_data[resolver_data::STAND2];
  else if (((!current->m_body_reliable &&
             current->m_mode == resolver::RESOLVE_BODY) ||
            current->m_mode == resolver::RESOLVE_WALK))
    mode_data = &m_resolver_data.m_mode_data[resolver_data::LBY_MOVING];
  ...
  for (auto i = 0; i < current->m_resolver_data.m_dir_data.size(); i++) {
    ...
    if (i == mode_data->m_index) {
      ...
      {
        current->m_fake_bones.push_back(static_cast<bone_array_t*>(
            g.m_interfaces->mem_alloc()->alloc(sizeof(bone_array_t) * 128)));
        if (!g_bones.setup(current->m_ent, current->m_fake_bones.back(),
                           current, &m_ik))
          all_setup = false;
      }
      bone_ix++;
